...
export default function UsersPage() {
  ...
  const [editingUser, setEditingUser] = useState<UserApiRow | null>(null);
  ...
  async function loadUsers(nextPage = page, nextPageSize = pageSize, filters = appliedFilters) {
    setLoading(true);

    try {
      const searchParams = buildUsersSearchParams(nextPage, nextPageSize, filters);
  ...
  const rows = useMemo<UserRow[]>(
    () => mapUsersToRows(users, (page - 1) * pageSize),
    [page, pageSize, users]
  );
  ...
  function handleEdit(row: UserRow) {
    setEditingUser(buildEditUser(row));
    ...
    try {
      ...
      toast({
        ...
        description: editingUser
          ? "The user changes have been saved."
    ...
    <DashboardShell title={pageTitle} description={pageDescription}>
      <DynamicPage
        ...
        onApplyFilters={(values) => {
          ...
          void loadUsers(1, pageSize, values);
        }}
        onResetFilters={() => {
          const cleared = {};
          setDraftFilters(cleared);
          setAppliedFilters(cleared);
          void loadUsers(1, pageSize, cleared);
        }}
        currentPage={page}
        ...
        onPageChange={(nextPage) => {
          void loadUsers(nextPage, pageSize, appliedFilters);
        }}
        onPageSizeChange={(nextPageSize) => {
          void loadUsers(1, nextPageSize, appliedFilters);
        }}
        serverSideFiltering
            ...
            <div className="flex items-start justify-between gap-6">
              <div>
                ...
                <p className="mt-1 text-[14px] text-[#6b7280] dark:text-[#94a3b8]">
                  Download the template, fill your user data, then upload the CSV file.
                </p>
              </div>
              <button
                type="button"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/18 p-4 backdrop-blur-[2px] dark:bg-[#020617]/55">
          <div className="w-full max-w-[560px] rounded-[22px] border border-[#e5e7eb] bg-white px-6 py-6 text-[#111827] shadow-[0_20px_48px_rgba(15,23,42,0.16)] dark:border-white/10 dark:bg-[#111827] dark:text-white dark:shadow-[0_20px_52px_rgba(2,6,23,0.55)]">
            ...
            <form className="space-y-5" onSubmit={(event) => void handleSubmit(event)}>
              ...
              <div className="space-y-2.5">
                <Label className="text-[14px] font-medium text-[#374151] dark:text-[#d1d5db]">
                  Password {editingUser ? <span className="text-[#94a3b8]">(optional)</span> : null}
                </Label>
                <Input
                  type="password"
                  value={form.password}
              ...
              <div className="mt-8 flex items-center justify-end gap-4">
                ...
                <button
                  ...
                  className="rounded-[12px] bg-[#4438ff] px-5 py-2.5 text-[14px] font-medium text-white hover:bg-[#3c31ec] disabled:cursor-not-allowed disabled:opacity-70 dark:bg-[#5b61ff] dark:hover:bg-[#6970ff]"
                >
                  {submitting ? "Saving..." : editingUser ? "Update" : "Save"}
                </button>
              </div>
            </form>
          </div>
